...
const sportsList = [...new Set(games.map(game => game.sport))];
// Objeto para rastrear as odds acumuladas
const accumulatedOdds = {};
...
function removeFromSelections(gameId) {
...
function displayOddAccumulated(accumulatedOdd) {
...
document.addEventListener('click', (event) => {
    ...
    if (teamElement && gameElement) {
        ...
        if (isSelectionAlreadyMade(gameId)) {
            ...
            const team = ((_a = teamElement.textContent) === null || _a === void 0 ? void 0 : _a.trim()) || '';
...
function displayGames(gamesList = games) {
...
function displayBetSlipOnBoletim(betData) {
...
function clearBetSlip() {
    const betAmountInput = document.getElementById('amount');
...
function placeBet() {
    ...
    const accumulatedOdd = parseFloat((accumulatedOddText === null || accumulatedOddText === void 0 ? void 0 : accumulatedOddText.replace('Odd Acumulada: ', '')) || '0');
    ...
    const depositAmount = parseFloat((depositAmountText === null || depositAmountText === void 0 ? void 0 : depositAmountText.replace('Você tem em seu depósito R$', '')) || '0');
...
function subtractBetAmountFromParagraph(amount) {
    var _a;
    if (createInputDesp) {
        // Obtém o valor atual no parágrafo
        const currentAmount = parseFloat(((_a = createInputDesp.textContent) === null || _a === void 0 ? void 0 : _a.replace('Você tem em seu depósito R$', '')) || '0');
...
function saveBetDataToLocalStorage(betData) {
    const storedBetData = localStorage.getItem('betData');
...
const valoresDepositados = [];
const divDesp = document.getElementById('deposito');
...
function Depositar() {
    var _a;
    const inputDesp = document.getElementById('amountDesp');
    if (!inputDesp) {
